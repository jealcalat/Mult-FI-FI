    ...
    if(!is.null(df_t)){
      ...
      b1b2 <- lapply(cde,function(l){
        ...
        df_cdetr <- lapply(n_trials,function(j){ # second lapply
          ...
          met <- p_wise(r_times) # returns a vector of s1,s2,spread,mid,r1,r2,r3
          if(!is_null(met)){
            names(met) <- c("start","stop","spread","middle","r1","r2","r3")
            met_trial <- t(met) %>% as.data.frame() # makes named met a df
            met_trial$trial <- j
            met_trial
          } else {
            met_trial <- rep(NA,8) 
            names(met_trial) <- c("start","stop","spread","middle","r1","r2","r3")
