# For overal response rate
# aco = 1 for phase yoked
read_sep_by_lever_phase <- function(aco){
  
  if(aco == 1){
    setwd("~/Dropbox/mult_IF_aco_exp/bck_IF_mult_all/Acoplado")
    sj_list <- list.files(pattern = paste0("^SM(.*).txt"),recursive = F)
  } else {
    setwd("~/Dropbox/mult_IF_aco_exp/bck_IF_mult_all/Doble_pico")
    sj_list <- list.files(pattern = paste0("^SM(.*).txt"),recursive = F)
  }
  
  df_sj <- lapply(sj_list,function(sj){
